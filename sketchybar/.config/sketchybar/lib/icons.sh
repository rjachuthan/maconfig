...
# Path to icon map script (generated by sketchybar-app-font)
ICON_MAP_SCRIPT="$CONFIG_DIR/helpers/icon_map.sh"

# Source the icon map function once
...
get_app_icon() {
  ...
  # Check if icon map function exists
  if ! declare -f __icon_map >/dev/null 2>&1; then
    echo ":default:"
    return 1
  fi

  # Call icon map function
  __icon_map "$app_name"
  echo "$icon_result"
}
